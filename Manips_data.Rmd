...
enriched_hal <- rbind(enriched_hal_score, enriched_hal_token) %>% distinct() %>%
    group_by(hal_id) %>% mutate(is_token_match = case_when(any(method == "token") ~ 1, TRUE ~ 0), #la communication a-t-elle au moins un match par token ?
                                is_score_0 = case_when(any(distance == 0) ~ 1, TRUE ~ 0)) %>% #la communication a-t-elle au moins un score à 0 ?
    ungroup() %>% mutate(is_row_delete = case_when(is_score_0 == 1 & method == "score" & distance == 0 ~ 0,
                                                   is_score_0 == 1 & method == "token" ~ 1,
                                                   is_score_0 == 0 & is_token_match == 1 & method == "score" ~ 1, 
                                                   TRUE ~ 0)) %>% #la jointure vient-elle du token ?
    filter(is_row_delete == 0) %>% select(-c(is_token_match, is_score_0, is_row_delete)) # on filtre pour ne pas garder les jointures par score lorsque token est disponible

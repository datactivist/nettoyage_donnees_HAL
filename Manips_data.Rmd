...
hal_manip <- as.data.frame(gsub("[[:punct:]]", "", as.matrix(hal))) %>%  # retrait de tous les caractères spéciaux
    ...
    mutate(booktitle = tolower(booktitle), # minuscules
...
Dans un premier temps ce sont tous les caractères des titres des conférences HAL qui sont matchés avec ceux du CORE, à partir de quoi on obtient une distance correspondant au nombre de caractères **qui ne sont pas communs** au 2 intitulés (si l'intitulé est exactement le même, la distance sera donc de 0). Cette manipulation est réalisée sur la colonne qui donne une information sur le titre de conférence parfois confondu avec le nom des communications dans les données du HAL (*booktitle*), matchée avec le champ des données CORE indiquant le nom de conférence (*title*).
...
Cette première passe de jointure inexacte sur les intitulés de conférences a permis d'enrichir `r n_distinct(enriched_hal_score$hal_id)` communications du HAL sur `r nrow(hal)` entrées, soit `r round(n_distinct(enriched_hal_score$hal_id) / nrow(hal) *100, 0)`%. Parmi ces jointures, `r enriched_hal_score %>% filter(distance == 0) %>% nrow()` sont exactes, c'est-à-dire que l'intitulé dans les données HAL correspond **exactement** à celui de la conférence dans les données CORE (en ayant retiré les caractères spéciaux et les majuscules). 
...
La deuxième jointure est réalisée en extrayant l'acronyme du nom de conférence dans les données HAL, puis chacun d'eux est matché avec les acronymes des données CORE. Si un mot des intitulés HAL est **exactement le même** que l'acronyme CORE, alors l'entrée est enrichie du référentiel CORE. L'extraction de l'acronyme est fait en plusieurs temps :
...
hal_manip_token <- hal %>% rowwise() %>% 
    mutate(first_word = word(booktitle, 1), #premier mot du string
...
hal_manip_token <- as.data.frame(gsub("[[:punct:]]", "", as.matrix(hal_manip_token))) %>%  # retrait de tous les caractères spéciaux
    mutate(hal_acronym = tolower(hal_acronym)) %>%  # minuscules
    mutate(hal_acronym = strsplit(as.character(hal_acronym), " ")) %>% unnest(hal_acronym) %>%  #split par ligne les acronymes mutliples
    mutate_all(na_if, "") %>% 
    mutate_all(na_if, " ") %>% 
...
enriched_hal_token_acro <- hal_manip_token %>% 
    ...
    mutate(method = "token acronym") %>% 
    select("hal_title","year","hal_id","booktitle","core_title","Acronym","Source","Rank","Primary FoR","method","core_id")

# On remet les noms initiaux (non lémmatisés)
enriched_hal_token_acro <- enriched_hal_token_acro %>% 
    # Noms des communications et conférences dans HAL
    select(-c(hal_title, booktitle)) %>% 
    left_join(., hal %>% select(title, booktitle, hal_id) %>% mutate(hal_id = str_replace_all(hal_id, "-", "")), by = "hal_id") %>% 
    rename(hal_title = title) %>% 
    ...
    select("hal_title","year","hal_id","booktitle","core_title","Acronym","Source","Rank","Primary FoR","method")
        ...
        #--------- DEUXIÈME PASSE DE JOINTURE TOKEN


# On filtre sur les communications qui n'ont pas été enrichies
enriched_hal_token_3char <- left_join(hal_manip, enriched_hal_token_acro %>% select(hal_id, Source), by = "hal_id") %>% 
    filter(is.na(Source)) %>% 
    select(-Source) %>% 
    mutate(split = strsplit(as.character(booktitle), " ")) %>% unnest(split) %>% 
    filter(!is.na(split), nchar(split) >= 3) %>% 
    left_join(., 
              core_manip %>% select(core_title, core_acronym, Acronym, core_id, Source, Rank, `Primary FoR`), 
              by = c("split" = "core_acronym")) %>% 
    filter(!is.na(Acronym)) %>% 
    mutate(method = "token 3 char")

# On remet les noms initiaux (avec caractères spéciaux et majuscules)
enriched_hal_token_3char <- enriched_hal_token_3char %>% 
    # Noms des communications et conférences dans HAL
    select(-c(hal_title, booktitle)) %>% 
    left_join(., hal %>% select(title, booktitle, hal_id) %>% mutate(hal_id = str_replace_all(hal_id, "-", "")), by = "hal_id") %>% 
    rename(hal_title = title) %>% 
    ...
    select("hal_title","year","hal_id","booktitle","core_title","Acronym","Source","Rank","Primary FoR","method")
        ...
        #--------- FINALISATION JOINTURE TOKEN



# 0n remet les 2 jointures ensemble
enriched_hal_token <- rbind(enriched_hal_token_acro, enriched_hal_token_3char) %>% distinct() %>% 
    mutate(distance = NA_real_, .before = "method")

# On remet avec la jointure par distance
enriched_hal <- rbind(enriched_hal_score, enriched_hal_token) %>% select(-c(method, distance)) %>% distinct()

    
# Stats
a <- enriched_hal_token %>% filter(method=="token 3 char")
a <- data.frame(table(a$Acronym)) %>% arrange(desc(Freq))
n_distinct(enriched_hal$hal_id) #9642 communications enrichies des données CORE
```
...
hist(table$n, main = "Distribution du nombre d'acronymes matchés par communication")
```

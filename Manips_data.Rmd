...
- le premier mot est récupéré (excepté s'il s'agit des 3 mots suivants : IEEE, ACM, SIAM);
- le ou les mots en lettres capitales sont récupérés ;
- un arbitrage est effectué entre les 2 valeurs récupérées, les commandes sont visibles dans le script après ces lignes.

Après jointure des acronymes extraits des noms de conférences HAL avec les données du CORE, les données sont réharmonisées avec les bases initiales d'où l'on récupère les intitulés et acronymes non lemmatisés. 
...
Cette deuxième passe de jointure exacte par mot commun à l'acronyme CORE, a permis d'enrichir `r n_distinct(enriched_hal_token$hal_id)` communications du HAL, dont `r length(setdiff(enriched_hal_token$hal_id, enriched_hal_score$hal_id))` qui n'avaient pas été enrichies par la première passe de match sur les chaînes de caractères (score).
...
Finalement, nous regroupons les données de communications enrichies via la méthode de score ou via la méthode de token dans une même base de données. Lorsqu'une proposition de référentiel CORE est disponible avec les 2 méthodes pour une communication HAL, c'est la méthode de token qui est préférée. Ainsi, nous avons au total `r n_distinct(a$hal_id)` communications harmonisées et enrichies par les données du CORE, soit `r round(n_distinct(a$hal_id)/nrow(hal)*100, 0)`% des données HAL récupérées par les années de 2018 à 2022. Parmi ces `r n_distinct(a$hal_id)` communications enrichies, `r n_distinct(b$hal_id)` sont finalement issues de la méthode du score (match inexact entre les intitulés de conférence) et `r n_distinct(c$hal_id)` de la méthode du token (match exact entre les acronymes de conférence).

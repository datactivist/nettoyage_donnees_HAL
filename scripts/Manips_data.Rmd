...
Un identifiant unique (*hal_id*) est créé temporairement pour les `r a` communications n'ayant pas d'identifiant à l'import des données de l'archive ouverte.
...
- dans tous les autres cas, le rang le plus récent est appliqué.

Ainsi, le champ *'Source'* est mis à jour en fonction de cette règle. Pour `r n_distinct(a$core_id)` communications, la source des données CORE non millésimées ne correspondait pas à l'année de la communication HAL, et donc a été mise à jour. Parmi ces dernières, le rang change pour `r n_distinct(b$core_id)` d'entre elles. 
